...
export function createApiClient(
  ...
  async function request<TResponse>(
    ...
    // Dev-only timing: splits "how long did signing this request take" from "how long did the
    // network round trip take", so a slow page load can be attributed to MSAL token acquisition
    // vs. the actual HTTP call instead of guessed at. Safe to leave in -- it's a no-op in a
    // production build and never runs in a non-dev environment.
    const isTimingEnabled = import.meta.env.DEV;
    const requestStartedAt = isTimingEnabled ? performance.now() : 0;

    const accessToken = await getAccessToken();         //gets the logged-in user's access token

    const tokenAcquiredAt = isTimingEnabled ? performance.now() : 0;
    ...
    if (isTimingEnabled) {
      ...
      const tokenMs = Math.round(tokenAcquiredAt - requestStartedAt);
      const fetchMs = Math.round(responseReceivedAt - tokenAcquiredAt);
      const totalMs = Math.round(responseReceivedAt - requestStartedAt);
      // eslint-disable-next-line no-console
      console.debug(
        `[api] ${init.method ?? "GET"} ${path} -> ${response.status} ` +
          `(token ${tokenMs}ms, network ${fetchMs}ms, total ${totalMs}ms)`,
      );
    }
