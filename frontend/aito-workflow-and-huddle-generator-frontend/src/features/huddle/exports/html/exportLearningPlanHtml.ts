...
export function createLearningPlanHtmlExport(plan: HuddlePlanResponse, options: LearningPlanHtmlExportOptions = {}): HtmlExportFile {
  ...
  const body = `<main class="page"><header class="hero"><p class="eyebrow">Frontier Accelerator</p><h1>Role Path Learning Plan</h1><p class="hero-description">Seven API-backed Huddles for Weeks 6–12.</p><div class="meta"><span class="pill">${escapeHtml(plan.roleExternalId)}</span><span class="pill">Weeks 6–12</span><span class="pill">${items.length} Huddles</span>${plan.isCustomized ? `<span class="pill">Customized</span>` : ""}</div></header><div class="content"><section class="plan-summary"><div><span class="label">Audience</span><strong>${escapeHtml(plan.roleExternalId)}</strong></div><div><span class="label">Plan status</span><strong>${plan.isCustomized ? "Customized" : "Recommended"}</strong></div></section><section class="timeline">${items.map((item) => {
  ...
  return { html: createHtmlDocument(`${plan.roleExternalId} Learning Plan`, body), fileName: options.fileName ?? safeHtmlFileName(`${plan.roleExternalId} - Learning Plan`, "Learning Plan") };
}
