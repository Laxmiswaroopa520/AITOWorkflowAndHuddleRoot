...
export function createHuddleHtmlExport(model: HuddlePresentationModel, options: HuddleHtmlExportOptions = {}): HtmlExportFile {
  ...
  const body = `<main class="page v3-export segmented-huddle-page">
    <header class="page-topbar"><div class="microsoft-brand"><span class="ms-grid"><i></i><i></i><i></i><i></i></span><span>Microsoft</span></div><div class="ribbon-art"><i></i><i></i><i></i></div></header>
    ...
    <div class="content">
      ...
      <section class="panel discussion-panel" data-section-panel="discussion"><div class="required-section-heading"><div><p class="eyebrow">Huddle Activities</p><h2>Top Scenarios or JTBD with the Agent</h2><p>Select a workflow stage, then open an activity to view and copy its recommended prompt.</p></div><span class="scenario-count" data-visible-activity-count>${activities.length} activities</span></div>${phases.length ? `<div class="phase-filter">${phases.map((phase, index) => `<button type="button" class="${index === 0 ? "is-active" : ""}" data-phase-filter="${text(phase.externalId)}" data-phase-name="${text(phase.name)}" data-phase-description="${text(phase.description || "")}"><span>${index + 1}</span><strong>${text(phase.name)}</strong><small>${phase.activities.length} ${phase.activities.length === 1 ? "activity" : "activities"}</small></button>`).join("")}</div><div class="selected-phase"><span>1</span><div><small>Selected phase</small><h3 data-selected-phase-title></h3><p data-selected-phase-description></p></div></div>` : ""}<div class="activity-stack">${activities.length ? activities.map(({ activity, phase }, index) => `<details class="activity" data-activity-phase="${text(phase.externalId)}"${index === 0 ? " open" : ""}><summary><span class="activity-index">${index + 1}</span><span><small>${text(phase.name)}</small><strong>${text(activity.name)}</strong></span><b>⌄</b></summary><div class="activity-body">${activity.description ? `<p class="activity-copy">${text(activity.description)}</p>` : ""}${activity.prompt ? `<div class="prompt-label-row"><span class="label">Try this prompt</span><button type="button" class="copy-button" data-copy-prompt="${text(activity.prompt)}" aria-label="Copy prompt" title="Copy prompt">▣</button></div><div class="prompt-scroll">${text(activity.prompt)}</div>` : unavailable}<div class="activity-grid">${field("Expected output", activity.expectedOutput)}${field("Human checkpoint", activity.humanCheckpoint)}${field("Required context", activity.requiredContext)}${field("Best-fit job", activity.bestFitJob)}</div>${activity.agents.length ? `<div class="subsection"><span class="label">AI tools</span>${agentNames(activity.agents)}</div>` : ""}${activity.resources.length ? `<div class="subsection"><span class="label">Activity resources</span>${resourceMarkup(activity.resources)}</div>` : ""}</div></details>`).join("") : unavailable}<div class="empty-state" data-phase-empty hidden>No activities are mapped to this phase.</div></div></section>
      ...
      <section class="panel notes-panel" data-section-panel="notes"><div class="panel-heading"><span>04</span><div><p class="eyebrow">Governed content</p><h2>Facilitator Notes</h2></div></div>${guide ? `<div class="guide-grid">${field("Session introduction", guide.sessionIntroduction, true)}<div class="field"><span class="label">Key talking points</span>${list(guide.keyTalkingPoints)}</div><div class="field"><span class="label">Discussion questions</span>${list(guide.discussionQuestions)}</div><div class="field"><span class="label">Suggested transitions</span>${list(guide.suggestedTransitions)}</div>${field("Wrap-up guidance", guide.wrapUpGuidance, true)}</div>` : unavailable}</section>
