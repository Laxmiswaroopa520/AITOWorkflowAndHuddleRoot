...
const aiAssistanceLabels = [
  "Suggest an icebreaker",
  "Generate an alternate prompt",
  "Summarize the current phase",
  "Show common pitfalls",
] as const;

function AiAssistanceActions() {
  return (
    <section className="rounded-xl border border-[#c7e0f4] bg-[#f5f9ff] p-4">
      <h3 className="font-semibold text-[#0f6cbd]">AI assistance</h3>
      <p className="mt-1 text-xs leading-5 text-[#616161]">AI assistance is unavailable because no approved AI backend is configured.</p>
      <div className="mt-3 grid gap-2">
        {aiAssistanceLabels.map((label) => <Button key={label} variant="outline" className="justify-start bg-white" disabled title="AI assistance unavailable"><Sparkles className="mr-2 h-4 w-4" />{label}</Button>)}
      </div>
    </section>
  );
}
