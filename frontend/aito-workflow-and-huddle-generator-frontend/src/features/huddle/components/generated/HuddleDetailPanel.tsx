...
export function HuddleDetailPanel({ data, isLoading, error, hasSelection, onRetry, onOpenWorkspace: openWorkspace, onPreview, onExportHtml, onExportPowerPoint, onMeetCoach }: HuddleDetailPanelProps) {
  // `exportPending` stays declared on the props interface below (callers still pass it) but is no
  // longer read here -- PowerPoint export is temporarily disabled, see the "Download PPT" menu item.
  const [activeTab, setActiveTab] = useState<DetailTab>("overview");
  const [secondaryOpen, setSecondaryOpen] = useState(false);
  // PowerPoint export is temporarily disabled (design/content not finalized yet) -- see the
  // matching comment in HuddlePage.tsx. Clicking "Download PPT" flashes this instead of
  // generating anything, so the button stays visible and wired without doing real work.
  const [pptUnavailable, setPptUnavailable] = useState(false);
