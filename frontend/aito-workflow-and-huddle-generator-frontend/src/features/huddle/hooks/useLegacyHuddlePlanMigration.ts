...
export function useLegacyHuddlePlanMigration(role: string | null, plan?: HuddlePlanResponse, catalog?: HuddleCatalogItemResponse[]) {
  ...
  useEffect(() => {
    // An empty plan means the role has no weekly path, so there is nothing to migrate into.
    if (!role || !plan || plan.items.length === 0 || !catalog || attempted.current.has(role)) return;
    ...
    try {
      ...
      const request = { roleExternalId: role, rowVersion: plan.rowVersion, items: ordered.map((item, index) => ({ week: index + 1, huddleExternalId: item.huddleId! })) };
