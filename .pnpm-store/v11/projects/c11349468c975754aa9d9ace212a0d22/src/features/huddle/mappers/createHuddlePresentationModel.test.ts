  ...
  return {
    ...
    phases: [
      ...
      { externalId: "phase-1", name: "Phase 1", description: null, durationMinutes: 20, displayOrder: 1, activities: [
        { externalId: "activity-2", name: "Activity 2", description: null, durationMinutes: 10, displayOrder: 2, prompt: "Prompt 2", expectedOutput: "Output 2", humanCheckpoint: "Check 2", requiredContext: "Context 2", bestFitJob: "Job 2", agents: [agent("activity-agent-2", 2), agent("activity-agent-1", 1)], resources: [resource("activity-resource-2", 2), resource("activity-resource-1", 1)] },
        { externalId: "activity-1", name: "Activity 1", description: null, durationMinutes: 10, displayOrder: 1, prompt: "Prompt 1", expectedOutput: "Output 1", humanCheckpoint: "Check 1", requiredContext: "Context 1", bestFitJob: "Job 1", agents: [], resources: [] },
      ] },
    ],
    facilitatorGuide: { sessionIntroduction: "Introduction", keyTalkingPoints: ["Point"], discussionQuestions: ["Question"], suggestedTransitions: ["Transition"], wrapUpGuidance: "Wrap up" },
...
describe("createHuddlePresentationModel", () => {
  it("maps a complete detail response using exact semantic fields", () => {
    ...
    expect(model.phases[0]?.activities[0]).toMatchObject({ requiredContext: "Context 1", bestFitJob: "Job 1" });
