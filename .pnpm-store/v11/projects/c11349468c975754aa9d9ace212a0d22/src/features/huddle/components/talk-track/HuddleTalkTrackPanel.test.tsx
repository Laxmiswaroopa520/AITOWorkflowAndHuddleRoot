...
describe("HuddleTalkTrackPanel", () => {
  ...
  it("keeps V3 AI-assistance labels visible but unavailable", () => {
    const markup = render(completeGuide);

    ["Suggest an icebreaker", "Generate an alternate prompt", "Summarize the current phase", "Show common pitfalls"].forEach((label) => expect(markup).toContain(label));
    expect(markup).toContain("no approved AI backend is configured");
