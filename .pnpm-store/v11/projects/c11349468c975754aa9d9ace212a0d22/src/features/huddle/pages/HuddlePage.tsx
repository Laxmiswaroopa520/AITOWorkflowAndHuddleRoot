...
const navigationItems: { id: HuddleViewMode; label: string }[] = [
  { id: "orientation", label: "Onboarding" },
...
export function HuddlePage() {
  ...
  return (
    <div className="mx-auto max-w-[1540px] space-y-6 p-4 [font-family:'Segoe_UI_Variable','Segoe_UI',Arial,sans-serif] lg:p-6">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><div className="mb-2 flex items-center gap-3"><span className="rounded-xl bg-[#E8F2FF] p-2"><Users className="h-6 w-6 text-[#0F6CBD]" /></span><h1 className="text-2xl font-bold lg:text-3xl">Run a Huddle</h1></div><p className="text-muted-foreground">Discover and run guided Huddles that help your team apply AI to real workflows.</p></div><div className="flex flex-wrap items-center gap-2">{persona === "manager" && <button type="button" onClick={() => navigate("/huddle/launch-planner")} className="inline-flex h-10 items-center rounded-lg border border-[#0A6BBA] bg-white px-4 text-sm font-semibold text-[#0A6BBA] hover:bg-[#E2F1F9]"><CalendarDays className="mr-2 h-4 w-4" />Launch Planner</button>}<button type="button" onClick={() => setResourcesOpen(true)} className="inline-flex h-10 items-center rounded-lg border bg-white px-4 text-sm font-semibold hover:bg-muted"><Library className="mr-2 h-4 w-4" />Resources</button>{viewMode !== "orientation" && <span className="inline-flex h-10 items-center rounded-lg border border-[#0F6CBD]/25 bg-[#E8F2FF] px-3 text-sm font-semibold text-[#0F6CBD]">{selectedExternalId ? "1 selected" : "0 selected"}</span>}{selectedExternalId && <button type="button" onClick={() => setWorkspaceOpen(true)} className="inline-flex h-10 items-center rounded-lg bg-[#0F6CBD] px-4 text-sm font-semibold text-white shadow-lg shadow-[#0F6CBD]/20 hover:bg-[#115EA3]"><Sparkles className="mr-2 h-4 w-4" />Generate Huddle</button>}</div></header>
      {persona && <div className="space-y-4"><nav aria-label="Huddle sections" className="inline-flex flex-wrap items-center gap-1 rounded-xl border border-border/80 bg-muted/40 p-1.5 shadow-sm">{navigationItems.map((item) => <button key={item.id} type="button" onClick={() => changeViewMode(item.id)} className={cn("rounded-lg px-5 py-2 text-sm font-semibold transition-all duration-200", viewMode === item.id ? "border border-[#0F6CBD] bg-[#0F6CBD] text-white shadow-md shadow-[#0F6CBD]/20" : "text-muted-foreground hover:bg-white/80 hover:text-foreground")}>{item.label}</button>)}</nav>{viewMode === "guided" && <HuddleAudienceSelect roles={roles} value={selectedRoleExternalId} onChange={(value) => { setSelectedRoleExternalId(value); setSelectedExternalId(null); }} />}</div>}
