...
*/
/* ---------------------------------------------------------------------------------------------
   MERGED for fresh-install-v11.78 (2026-09-16): this script now also contains the new-only
   Huddle rows identified by comparing the 9.15.2026 Frontier Accelerator workbook against the
   V10.2.1 workbook this folder was originally built from. Every row already present in
   fresh-install-v10.2.1 is untouched; only rows verified absent from that baseline were appended,
   using the same ExternalId-keyed MERGE already in this file. See the new 00_README_Fresh_Install.md
   in this folder for the full merge report and the list of items deliberately NOT included.
   --------------------------------------------------------------------------------------------- */
/* ---------------------------------------------------------------------------------------------
   MERGED for fresh-install-v11.78 (2026-09-17): appended 2 new HuddleTopicAgents rows to the
   Topic-to-agent MERGE's VALUES list only -- (WF-X-DEAL-01, AGT-016) and (WF-X-RENEW-01, AGT-016)
   -- matching the already-executed huddle-2026.09.16-update/02_Seed_New_Deal_Agent_Topic_Joins.sql.
   DisplayOrder is hardcoded here (9 and 10 respectively) rather than computed dynamically, since
   this is a static fresh-install seed for a brand-new database with no prior rows to compute MAX
   against; these are the verified next-free values against every other row already in this same
   VALUES list for these two topics (WF-X-DEAL-01 previously maxed at 8, WF-X-RENEW-01 at 9).
   Per the established precedent already used in this file's 2026-09-16 merge above, only the
   MERGE statement's own VALUES list is extended -- the separate validation SELECT/THROW block
   immediately below (expected-count 63) is intentionally left unchanged, exactly as that block was
   already left unchanged when the 2026-09-16 merge added its own new rows. All other rows and all
   other MERGE blocks in this file (McemStages, TopicResources, AgentResources) are untouched. The
   unexecuted, optional existing-data corrections from huddle-2026.09.16-update/
   03_OPTIONAL_Existing_Data_Corrections.sql are deliberately NOT included here.
   --------------------------------------------------------------------------------------------- */
...
;WITH raw AS (
    ...
    ,
        ...
        (N'WF-X-DEAL-01', N'AGT-016', N'Primary', N'Deal Agent, surfaced through Sales Agent', 0, 9),
        (N'WF-X-RENEW-01', N'AGT-016', N'Primary', N'Deal Agent, surfaced through Sales Agent', 0, 10)) v([TopicExternalId], [AgentExternalId], [UsageType], [DisplayLabel], [ShowAgentAccessLink], [DisplayOrder])
), src AS (
...
;WITH raw AS (
    SELECT * FROM (VALUES
        ...
        (N'WF-X-USECASE-01', N'RES-GLOBAL-02', 2),
        (N'WF-X-ASSET-01', N'RES-GLOBAL-02', 2)
...
SELECT @unresolved = 21 - COUNT(*) FROM raw
    INNER JOIN dbo.HuddleTopics AS t ON t.[ExternalId] = raw.[TopicExternalId]
    INNER JOIN dbo.HuddleResources AS rs ON rs.[ExternalId] = raw.[ResourceExternalId];
IF @unresolved <> 0
    THROW 51000, 'HuddleTopicResources: 21 source rows were expected to resolve their foreign keys but some did not. Run the earlier scripts in order and re-check the workbook.', 1;
...
;WITH raw AS (
    SELECT * FROM (VALUES
        ...
        (N'AGT-001', N'RES-050', 8)
    ,
        (N'AGT-008', N'RES-007', 1),
        (N'AGT-013', N'RES-SALESHOME', 1),
        (N'AGT-014', N'RES-KYP', 1),
        (N'AGT-013', N'RES-GLOBAL-01', 2),
        (N'AGT-014', N'RES-GLOBAL-01', 2)) v([AgentExternalId], [ResourceExternalId], [DisplayOrder])
), src AS (
...
SELECT @unresolved = 23 - COUNT(*) FROM raw
    ...
    INNER JOIN dbo.HuddleResources AS rs ON rs.[ExternalId] = raw.[ResourceExternalId];
IF @unresolved <> 0
    THROW 51000, 'HuddleAgentResources: 23 source rows were expected to resolve their foreign keys but some did not. Run the earlier scripts in order and re-check the workbook.', 1;
