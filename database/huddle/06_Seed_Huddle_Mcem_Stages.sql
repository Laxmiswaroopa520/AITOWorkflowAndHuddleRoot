...
BEGIN TRY
    ...
    DECLARE @Source TABLE (ExternalId nvarchar(100) NOT NULL, Name nvarchar(200) NOT NULL, Description nvarchar(2000) NULL);
    INSERT @Source VALUES
(N'MCEM-01',N'Listen and Consult',NULL),
(N'MCEM-02',N'Inspire and Design',NULL),
(N'MCEM-03',N'Empower and Achieve',NULL),
(N'MCEM-04',N'Realize Value',NULL),
(N'MCEM-05',N'Manage and Optimize',NULL);
    ...
    SELECT @UpdatedCount=COUNT(*) FROM dbo.HuddleMcemStages t JOIN @Source s ON s.ExternalId=t.ExternalId;
    UPDATE t SET t.Name=s.Name,t.Description=s.Description, t.UpdatedAtUtc=SYSUTCDATETIME() FROM dbo.HuddleMcemStages t JOIN @Source s ON s.ExternalId=t.ExternalId;
    INSERT dbo.HuddleMcemStages (ExternalId,Name,Description,CreatedAtUtc,UpdatedAtUtc)
    SELECT s.ExternalId,s.Name,s.Description,SYSUTCDATETIME(),NULL FROM @Source s WHERE NOT EXISTS (SELECT 1 FROM dbo.HuddleMcemStages t WHERE t.ExternalId=s.ExternalId);
