...
BEGIN
    RAISERROR('FAIL: the schema script has not been applied. Run 01_Schema_All_Migrations.sql first.', 16, 1);
    RETURN;
END;
...
PRINT '';
PRINT 'Preflight complete. If every check said OK, continue with 10_Workflow_Roles.sql (see 00_README for the full run order).';
