...
import { Bot, Clock, ExternalLink, MoreHorizontal, Presentation, Target, Users } from "lucide-react";
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="block w-full text-left">
            ...
            <span className="mt-2 flex flex-wrap items-center gap-3 text-xs text-muted-foreground"><span className="flex items-center gap-1"><Clock className="h-3 w-3" />{huddle.durationMinutes === null ? "Duration unavailable" : `${huddle.durationMinutes} min`}</span><span className="flex items-center gap-1"><Users className="h-3 w-3" />{huddle.audienceLabel}</span>{huddle.focusArea && <span>{huddle.focusArea}</span>}{huddle.mcemStageLabel && <span className="flex items-center gap-1"><Target className="h-3 w-3" />{huddle.mcemStageLabel}</span>}</span>
