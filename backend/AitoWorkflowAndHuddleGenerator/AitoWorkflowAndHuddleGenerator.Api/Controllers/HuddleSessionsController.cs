...
public sealed class HuddleSessionsController(ISender sender) : ControllerBase
{
    ...
    /// Gets Mine.
    /// </summary>
    public async Task<ActionResult<HuddleSessionResponse>> GetMine(string externalId, [FromQuery] string? placementExternalId, CancellationToken cancellationToken)
    {
        HuddleSessionResponse? response = await sender.Send(new GetMyHuddleSessionQuery(externalId, placementExternalId), cancellationToken);
    ...
    /// Saves Mine.
    /// </summary>
    public async Task<ActionResult<HuddleSessionResponse>> SaveMine(string externalId, [FromQuery] string? placementExternalId, [FromBody] SaveHuddleSessionRequest request, CancellationToken cancellationToken) =>
        Ok(await sender.Send(new SaveHuddleSessionCommand(externalId, request.CurrentPhaseExternalId, request.FacilitatorNotes, request.RowVersion, placementExternalId), cancellationToken));
    ...
    /// </summary>
    public async Task<ActionResult<HuddleSessionResponse>> SetActivityCompletion(string externalId, string activityExternalId, [FromQuery] string? placementExternalId, [FromBody] SetHuddleActivityCompletionRequest request, CancellationToken cancellationToken) =>
        Ok(await sender.Send(new SetHuddleActivityCompletionCommand(externalId, activityExternalId, request.IsCompleted, request.RowVersion, placementExternalId), cancellationToken));
    ...
    /// </summary>
    public async Task<ActionResult<HuddleSessionResponse>> Complete(string externalId, [FromQuery] string? placementExternalId, [FromBody] CompleteHuddleSessionRequest request, CancellationToken cancellationToken) =>
        Ok(await sender.Send(new CompleteHuddleSessionCommand(externalId, request.RowVersion, placementExternalId), cancellationToken));
