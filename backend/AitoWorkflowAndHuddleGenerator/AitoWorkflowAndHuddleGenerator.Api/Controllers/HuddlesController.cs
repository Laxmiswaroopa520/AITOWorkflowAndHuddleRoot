...
public sealed class HuddlesController : ControllerBase
{
    ...
    /// Optional placement to read. Supply it and the phases, activities and facilitator guide are
    /// scoped to one role's appearance of the Huddle. Omit it and the response aggregates every
    /// role that shares the topic.
    /// </param>
    /// <param name="cancellationToken">A token used to cancel the request.</param>
    ...
    public async Task<ActionResult<HuddleDetailResponse>> GetById(
        string externalId,
        [FromQuery] string? placementExternalId,
        CancellationToken cancellationToken)
    {
        HuddleDetailResponse response = await sender.Send(
            new GetHuddleByIdQuery(externalId, placementExternalId), cancellationToken);
        return Ok(response);
    }
}
