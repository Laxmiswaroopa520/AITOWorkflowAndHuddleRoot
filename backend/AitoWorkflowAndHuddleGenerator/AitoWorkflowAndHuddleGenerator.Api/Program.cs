...
// Diagnostic only: logs end-to-end time spent inside this process for each request, so a slow
// page load can be split into "slow in the API" vs. "slow in the browser/network/token
// acquisition". Registered first so its Stopwatch wraps everything below it. Development-only
// and safe to remove once the roles/audience performance investigation is done.
if (app.Environment.IsDevelopment())
{
    app.UseMiddleware<AitoWorkflowAndHuddleGenerator.Api.Middleware.RequestTimingMiddleware>();
}
...
// Compresses JSON responses. Must run before anything that writes a body.
