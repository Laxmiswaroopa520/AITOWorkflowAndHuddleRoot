...
public static class HuddleMessages
{
    ...
    public const string PlanWeeksInvalid = "The plan must contain exactly one item for each week on the role path, numbered from 1 with no gaps.";
    public const string PlanHuddlesMustBeUnique = "The plan must not use the same Huddle placement in more than one week.";
    public const string PlanPlacementUnknown = "One or more selected Huddle placements do not belong to this role path.";
    ...
    public const string ActivityNotCompletable = "Extended activities are optional and cannot be marked complete.";
    ...
    public static string PlacementNotFound(string placementExternalId, string topicExternalId) =>

        $"The placement '{placementExternalId}' was not found for Huddle '{topicExternalId}'.";
    ...
    /// </summary>
    /// <summary>
    /// Executes the Role Path Not Configured operation.
    /// </summary>
    public static string RolePathNotConfigured(string roleExternalId) =>
        $"No Role Path is configured for role '{roleExternalId}'. The workbook defines a weekly path "
        + "in Role_Paths; this role appears only in Additional_Content.";

    /// <summary>
    /// Executes the Role Path Not Contiguous operation.
    /// </summary>
    public static string RolePathNotContiguous(string roleExternalId, int weekCount) =>
        $"The Role Path for role '{roleExternalId}' must be a contiguous run of weeks starting at 1, "
        + $"but {weekCount} placement(s) are configured. Check the workbook's Role_Paths rows for this role.";
