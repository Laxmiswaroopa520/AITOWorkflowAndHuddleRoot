...
public sealed class ExceptionHandlingMiddlewareTests
{
    ...
    //
    // NOTE: the sensitive payload below is deliberately synthetic (no real credential shape, e.g.
    // "User Id=...;Password=...") so that pushing this file does not trip repository secret
    // scanning (Azure DevOps SEC101/037 SqlLegacyCredentials). Keep it that way -- do not restore
    // a realistic-looking username/password pair here, including inside comments, since scanners
    // read raw file text and do not understand that a line is commented out.
    [Fact]
    public async Task InvokeAsync_HidesSensitiveDetailFromClientButLogsTheRealExceptionServerSide()
    {
        var sensitiveException = new InvalidOperationException(
            "Failed executing DbCommand. Connection details: " +
            "Server=test-database;Database=AitoTest;CredentialMarker=[REDACTED-TEST-VALUE];");
        ...
        // Client: generic response only. No connection string, no credential marker, no exception
        // type name, regardless of environment -- this middleware has no
        ...
        Assert.DoesNotContain("[REDACTED-TEST-VALUE]", rawBody);
        Assert.DoesNotContain("CredentialMarker=", rawBody);
        Assert.DoesNotContain("test-database", rawBody);
        ...
        Assert.Contains("[REDACTED-TEST-VALUE]", errorEntry.Exception!.Message);
    }
